...
class Country extends Structure\Model
{
	...
	{
		return self::findAllByQuery(QueryHandler::select("*")->from(static::_TABLE)->where("`" . static::_NAME . "` LIKE :name")->addParameter('name', $name)) ?? [];
	}

	/**
	 * @return static[]
	 */
	public static function findByIsoCode2(string $isoCode2): array
	{
		return self::findAllByQuery(QueryHandler::select("*")->from(static::_TABLE)->where("`" . static::_ISO_CODE_2 . "` = :iso_code_2")->addParameter('iso_code_2', $isoCode2)) ?? [];
	}

	/**
	 * @return static[]
	 */
	public static function findByIsoCode3(string $isoCode3): array
	{
		return self::findAllByQuery(QueryHandler::select("*")->from(static::_TABLE)->where("`" . static::_ISO_CODE_3 . "` = :iso_code_3")->addParameter('iso_code_3', $isoCode3)) ?? [];
	}
}
